...
- **`patches/tools.py`** (modified). Adds an `invoke_skill` tool to the chat's tool set. The model calls it with a `skill` parameter (`allium`, `elicit`, `tend`, `weed`); dispatch reads the canonical SKILL.md from `/code/allium-skills/{skill}/SKILL.md` (mounted from `~/code/allium/skills/` on the host) and returns the content wrapped in a `search_library`-shaped result so views.py handles it without a new branch. The preamble prepended to the skill body instructs the model to treat the skill content as authoritative and override the system prompt where they conflict. This is what makes Beat 3's elicit moment use the real Allium skill rather than the primer's homemade approximation.
