...
def _dispatch_search_library(arguments: dict, user_message: str = "") -> dict | None:
    ...
    # Demo patch: the graph-based actions above match by case-insensitive
    # substring on names/ref_ids and miss natural-language framework refs
    # ("NY DFS Part 500" doesn't substring-match "NY DFS 500 with 2023-11
    # amendments"). When that happens, fall back to vector search over the
    # library partition so the LLM still sees relevant chunks.
    def _is_empty(d):
        if d is None:
            return True
        ...
        if isinstance(d, dict):
            for k in ("frameworks", "results", "requirements", "controls", "mappings"):
                v = d.get(k)
