...
def search(
    ...
    results_list = [
        {
            ...
            # Demo patch: expose filename for document chunks so format_context
            # can label them with their actual source rather than the generic
    ...
    # -----------------------------------------------------------------
    # Demo patch B: always include firm policy chunks
    # -----------------------------------------------------------------
    # Pure dense retrieval over a large library corpus (60k+ chunks) can
    # squeeze the firm's own policies out of the top-k for sparsely worded
    # queries — even when those policies are the most relevant material for
    # the question. Inject all document chunks in the user's accessible
    # folders as anchored context so the LLM never has to guess whether a
    # governance answer should be grounded in the firm's policy.
    #
    # Disable by commenting out the call below or by removing the volume
    # mount for patches/rag.py.
    if source_type is None and user is not None:
        try:
            anchor_chunks = _fetch_firm_policy_chunks(user)
            existing_ids = {r["id"] for r in results_list}
            anchors_to_prepend = [
                ac for ac in anchor_chunks if ac["id"] not in existing_ids
            ]
            # Prepend so the LLM sees firm policy text before less-relevant
            # library hits. The format_context function preserves order.
            results_list = anchors_to_prepend + results_list
        except Exception as e:
            logger.warning("firm_policy_anchor_fetch_failed", error=str(e))

    return results_list


def _fetch_firm_policy_chunks(user) -> list[dict]:
    """
    Pull every document chunk in folders the user can access. These are
    treated as canonical firm material and are included in chat context
    unconditionally, regardless of the dense-retrieval ranking.
    """
    ...
    accessible = get_accessible_folder_ids(user)
    if not accessible:
        return []
    ...
    points, _ = client.scroll(
        ...
        scroll_filter=Filter(
            must=[
                FieldCondition(
                    key="folder_id", match=MatchAny(any=accessible)
                ),
                FieldCondition(
                    key="source_type", match=MatchValue(value="document")
                ),
            ]
        ),
        limit=50,
    ...
    for p in points:
        ...
        out.append(
            {
                "id": str(p.id),
                "score": 0.0,  # anchor chunks aren't ranked; included regardless
                "text": pld.get("text", ""),
                "source_type": pld.get("source_type", ""),
                "object_type": pld.get("object_type", ""),
                "object_id": pld.get("object_id"),
                "name": pld.get("name", ""),
                "ref_id": pld.get("ref_id", ""),
                "framework": pld.get("framework", ""),
                "urn": pld.get("urn", ""),
                "filename": pld.get("filename", ""),
                "document_id": pld.get("document_id", ""),
            }
        )
    return out
...
def format_context(results: list[dict], expanded: list[dict] | None = None) -> str:
    ...
    for i, r in enumerate(results, 1):
        # Demo patch A: label document chunks with a human-readable identifier
        # (the filename stripped of the noisy prefix/suffix) rather than the
        # generic "document_chunk" object_type. Without this the LLM can't
        # tell from the header that what it's reading is the firm's policy.
        if r.get("source_type") == "document":
            fn = r.get("filename", "") or "document"
            source_label = _humanise_document_filename(fn)
        else:
...
def _humanise_document_filename(fn: str) -> str:
    """Strip 'policy-' prefix and '-revN.md' suffix from auto-generated filenames."""
    import re

    name = re.sub(r"^policy-", "", fn)
    name = re.sub(r"-rev\d+\.md$", "", name)
    return f"{name} (firm policy document)" if name != fn else name or fn
