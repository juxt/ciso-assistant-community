...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "- Never suggest downloading external files or tools — everything is done within the platform.\n"
...
TOOL_SYSTEM_PROMPT = (
    ...
    "2. propose_create — Propose creating new objects. ONLY use when the user EXPLICITLY asks to "
    "create, add, or import something. NEVER select this tool proactively.\n"
    ...
    "IMPORTANT: For follow-up questions ('on it', 'about that', 'dessus'), resolve the reference "
    "from the conversation history, NOT from the current page context.\n\n"
    "TOOL SELECTION RULES:\n"
    "- Questions about the user's data → query_objects\n"
    "- Questions about framework/standard content or structure → search_library\n"
    "- EXPLICIT requests to create or import (e.g. 'create', 'add', 'import') → propose_create\n"
    "- Informational questions about frameworks → search_library (NOT propose_create)\n"
    "- Requests to link/attach → attach_existing\n"
    "- Greetings or simple clarifications → no tool needed\n"
    "- Follow-up data questions ('do we have an audit on it?', 'how many?', 'what status?') "
    "→ USE a tool (query_objects/search_library), do NOT answer from previous context alone\n\n"
    "SAFETY:\n"
...
def _tool_selection_system_prompt(extra: str | None) -> str:
    """
    Compose the tool-selection system prompt. TOOL_SYSTEM_PROMPT carries the
    upstream tool-routing rules; an explicitly-set chat_system_prompt
    (e.g. the Allium primer in the wealth-firm demo) is appended so its
    domain-specific routing rules — for example "AI deployment submissions
    are documents in retrieved context, not records in the data model" —
    apply to the tool selection step rather than only to the final answer.

    DEFAULT_SYSTEM_PROMPT is excluded because it duplicates TOOL_SYSTEM_PROMPT's
    intent and is the value self.system_prompt holds when no custom chat
    prompt has been configured.
    """
    if extra and extra != DEFAULT_SYSTEM_PROMPT:
...
class OllamaLLM:
    ...
    def tool_call(
        ...
        messages = [{"role": "system", "content": _tool_selection_system_prompt(self.system_prompt)}]
        if history:
            for msg in history:
                messages.append({"role": msg["role"], "content": msg["content"]})
        messages.append({"role": "user", "content": prompt})
