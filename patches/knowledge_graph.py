...
# Demo patch: stopwords that must never resolve to a framework on their own.
# Without this, single tokens like "for" match CROE-for-FMI and "require"
# matches NIS 2's name substring, which then triggers auto_upgrade_to_compare
# and hijacks the chat away from the framework the user actually asked about.
_RESOLVE_STOPWORDS = frozenset({
    ...
    "what", "which", "when", "where", "why", "how", "who", "whose", "whom",
    "section", "article", "chapter", "part", "subpart", "title", "clause", "paragraph",
    "require", "requires", "required", "requirement", "requirements",
    "specify", "specifies", "specified", "control", "controls",
    "notice", "notify", "notification", "report", "reporting", "incident", "incidents",
    "under", "above", "below", "before", "after", "during",
    "about", "any", "all", "some", "other", "another", "such",
})
...
def _resolve_framework(G: DiGraph, identifier: str) -> str | None:
    ...
    # Demo patch: reject single-word common English stopwords to stop the
    # detector matching "for" -> CROE-for-FMI, "require" -> NIS2, etc.
    if " " not in identifier_lower and identifier_lower in _RESOLVE_STOPWORDS:
        return None
    ...
    # Demo patch: raise threshold from 0.25 to 0.5 so a 3-char token can no
    # longer hit exactly 0.25 against a 12-char ref_id and slip through.
    # Short legitimate abbreviations (e.g. "3CF") still resolve via direct
    # ref_id equality before we reach this scoring path.
    if best_score < 0.5:
        return None
    return best_match
